...
class TestMixer:
    ...
    def _make_mixer(self, mock_xair=None):
        ...
        with patch.dict("sys.modules", {"xair_api": mock_xair or MagicMock()}):
            with patch("shuffle_party.mixer.time"):
                return Mixer(host="192.168.1.100", port=10023, channels=[1, 2], fade_duration=3.0)
    ...
    def test_fade_out_sends_decreasing_values(self):
        ...
        # Extract values for channel 1 only (every other call is channel 2)
        ch1_calls = [c for c in mock_client.send.call_args_list if c.args[0] == "/ch/01/mix/fader"]
        values = [c.args[1] for c in ch1_calls]
    ...
    def test_fade_sends_to_both_stereo_channels(self):
        mock_xair = MagicMock()
        mock_client = MagicMock()
        mock_xair.connect.return_value = mock_client
        mixer = self._make_mixer(mock_xair)
        ...
        addresses = [c.args[0] for c in mock_client.send.call_args_list]
        assert "/ch/01/mix/fader" in addresses
        assert "/ch/02/mix/fader" in addresses
    ...
    def test_channel_numbers_are_zero_padded(self):
        ...
        with patch.dict("sys.modules", {"xair_api": mock_xair}):
            with patch("shuffle_party.mixer.time"):
                mixer = Mixer(host="x", port=1, channels=[5, 6], fade_duration=1.0)
                mixer.fade_out()

        addresses = set(c.args[0] for c in mock_client.send.call_args_list)
        assert addresses == {"/ch/05/mix/fader", "/ch/06/mix/fader"}
    ...
    def test_graceful_degradation_when_xr12_unreachable(self):
        ...
        with patch.dict("sys.modules", {"xair_api": mock_xair}):
            mixer = Mixer(host="x", port=1, channels=[1, 2], fade_duration=1.0)
